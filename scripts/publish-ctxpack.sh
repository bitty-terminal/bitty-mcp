...
# bitty-mcp checkout (the trigger is the merge closeout, NOT a git hook:
...
# bitty-terminal/bitty-mcp-workflow.
#
# What it does:
#   1. Exports `carryctx export --pack-format dir` from this repo into a
#      staging clone of bitty-terminal/bitty-mcp-workflow as
#      <UTC-date>-<mcp-main-sha>/.
...
#                  <repo>/../bitty-mcp-workflow).
#   --url URL      mirror remote for a fresh clone
#                  (default: $WORKFLOW_MIRROR_URL or
#                  https://github.com/bitty-terminal/bitty-mcp-workflow.git).
...
SOURCE_REPO="${WORKFLOW_SOURCE_REPO:-bitty-mcp}"
